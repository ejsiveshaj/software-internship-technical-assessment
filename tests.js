...
const testCases = [
  {
    message: "single digit strings can not be valid",
    input: "1",
    expected: 42,
  },
  ...
  {
    message: "valid number with an odd number of digits",
    input: "095 245 88",
    expected: 0,
  },
  {
    message: "valid number with an even number of digits",
    input: "234 567 891 234",
    expected: 0,
  },
  {
    message: "valid strings with a non-digit added at the end invalid",
    input: "059a",
    expected: 42,
  },
  {
    message: "valid strings with punctuation included become invalid",
    input: "055-444-285",
    expected: 42,
  },
  {
    message: "valid strings with symbols included become invalid",
    input: "055# 444$ 285",
    expected: 42,
  },
  ...
  {
    message: "input digit 9 is correctly converted to output digit 9",
    input: "091",
    expected: 0,
  },
  {
    message: "using ascii value for non-doubled non-digit isn't allowed",
    input: "055b 444 285",
    expected: 42,
  },
  {
    message: "using ascii value for doubled non-digit isn't allowed",
    input: ":9",
    expected: 42,
  },
];
...
for (const { input, expected, message } of testCases) {
  ...
  console.log("status from ts", status);
  ...
  if (expected !== status) {
    console.log(
      `❌  Input: "${input}", Status: ${status}, Expected: ${expected}\nℹ  Test description: ${message}`
    );
    process.exit(1);
  }
